    ...
    def __init__(self):
        self.steps_data = [
            ...
            [
                ...
                [
                    "-i",
                    f"%matches_dir%{FOLDER_DELIM}sfm_data.json",
                    "-o",
                    "%matches_dir%",
                    "-m",
                    "SIFT",
            ...
            [
                ...
                [
                    ...
                    f"%matches_dir%{FOLDER_DELIM}sfm_data.json",
                    "-o",
                    f"%matches_dir%{FOLDER_DELIM}pairs.bin",
                ],
            ],
            [
                "Compute matches",
                ...
                [
                    ...
                    f"%matches_dir%{FOLDER_DELIM}pairs.bin",
                    "-o",
                    f"%matches_dir%{FOLDER_DELIM}matches.putative.bin",
                    "-n",
                    "AUTO",
                ],
            ],
            [
                ...
                [
                    ...
                    f"%matches_dir%{FOLDER_DELIM}matches.putative.bin",
                    "-o",
                    f"%matches_dir%{FOLDER_DELIM}matches.f.bin",
                ],
            ],
            [
            ...
            [
                ...
                [
                    ...
                    "GLOBAL",
                    "-M",
                    f"%matches_dir%{FOLDER_DELIM}matches.e.bin",
                ],
            ],
            [
                "Colorize Structure",
                ...
                [
                    "-i",
                    f"%reconstruction_dir%{FOLDER_DELIM}sfm_data.bin",
                    "-o",
                    f"%reconstruction_dir%{FOLDER_DELIM}colorized.ply",
                ],
            ],
            [
                "Structure from Known Poses",
                os.path.join(
                    OPENMVG_BIN, "openMVG_main_ComputeStructureFromKnownPoses"
                ),
                [
                    "-i",
                    f"%reconstruction_dir%{FOLDER_DELIM}sfm_data.bin",
                    ...
                    f"%matches_dir%{FOLDER_DELIM}matches.f.bin",
                    "-o",
                    f"%reconstruction_dir%{FOLDER_DELIM}robust.bin",
                ],
            ],
            [
                ...
                [
                    ...
                    f"%reconstruction_dir%{FOLDER_DELIM}robust.bin",
                    "-o",
                    f"%reconstruction_dir%{FOLDER_DELIM}robust_colorized.ply",
                ],
            ],
            [
                "Control Points Registration",
                os.path.join(
                    OPENMVG_BIN, "ui_openMVG_control_points_registration"
                ),
                ["-i", f"%reconstruction_dir%{FOLDER_DELIM}sfm_data.bin"],
            ],
            [
                "Export to openMVS",
                ...
                [
                    ...
                    f"%reconstruction_dir%{FOLDER_DELIM}sfm_data.bin",
                    "-o",
                    f"%mvs_dir%{FOLDER_DELIM}scene.mvs",
                    "-d",
                    f"%mvs_dir%{FOLDER_DELIM}images",
                ],
            ],
            [
...
PARSER.add_argument(
    ...
    help=(
        (
            (
                "steps list preset in \r\n"
                + " \r\n".join([f"{k} = {str(PRESET[k])}" for k in PRESET])
            )
            + " \r\ndefault : "
        )
        + PRESET_DEFAULT
...
for n in range(STEPS.length()):
    GROUP.add_argument(f'--{str(n)}', nargs='+')
...
if not os.path.exists(CONF.input_dir):
    sys.exit(f"{CONF.input_dir}: path not found")
...
if CONF.steps and CONF.preset:
    ...
    try:
        ...
        sys.exit(f"Unknown preset {CONF.preset}, choose {' or '.join(list(PRESET))}")
elif not CONF.steps:
    CONF.steps = PRESET[PRESET_DEFAULT]

# WALK
print(f"# Using input dir:  {CONF.input_dir}")
print(f"#      output dir:  {CONF.output_dir}")
print(f"# Steps:  {str(CONF.steps)}")

if 4 in CONF.steps and 6 in CONF.steps:
        # Set the geometric_model of ComputeMatches to Essential
    STEPS.replace_opt(
        4, f"{FOLDER_DELIM}matches.f.bin", f"{FOLDER_DELIM}matches.e.bin"
    )
    STEPS[4].opt.extend(["-g", "e"])

if 15 in CONF.steps and 14 not in CONF.steps:
